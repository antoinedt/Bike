...
fun ConnectScreen(onBack: () -> Unit) {
    ...
    val discovered by trainer.discovered.collectAsStateWithLifecycle()
    ...
    val permissionLauncher = rememberLauncherForActivityResult(
        ...
        if (permissionsGranted) trainer.startScan()
    }
    ...
    Column(
        ...
        Text("Find your trainer", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold)
        Spacer(Modifier.height(4.dp))
        Text(
            when (connectionState) {
                TrainerConnectionState.Connected -> "Connected. You're ready to ride."
                TrainerConnectionState.Connecting -> "Connecting…"
                TrainerConnectionState.Failed -> "No supported trainer found. Make sure it's awake and pedalling."
                else -> "Scanning for FTMS / power trainers nearby…"
            },
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            TextButton(onClick = {
                if (permissionsGranted) trainer.startScan() else permissionLauncher.launch(requiredPermissions)
            }) {
                Text("Rescan")
            }
        }
        ...
        LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp)) {
            items(discovered, key = { it.address }) { device ->
                Card(
                    modifier = Modifier
                        .fillMaxWidth()
                        .clickable { trainer.connect(device.address) },
                ) {
                    Row(
                        modifier = Modifier
                            .fillMaxWidth()
